...
Then('a new tab should open to {string}', async function (expectedHost) {
    // if popup exists, assert its url contains expectedHost
    if (this._aboutPopup) {
        await this._aboutPopup.waitForLoadState('load', { timeout: 5000 }).catch(() => { });
        const url = this._aboutPopup.url();
        assert.ok(url.includes(expectedHost), `About popup url expected to include ${expectedHost} but was ${url}`);
        // close popup to keep environment clean
        await this._aboutPopup.close();
    } else {
        // fallback: maybe opened in same page; check current url
        const url = this.page.url();
        assert.ok(url.includes(expectedHost), `Expected current page url to include ${expectedHost} but was ${url}`);
        // if it's external and in same page, optionally navigate back
