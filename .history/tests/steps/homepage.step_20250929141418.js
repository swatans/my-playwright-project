...
// tests/steps/homepage.step.js (ganti fungsi yang mengecek product names)
Then('all product names start with {string}', async function (prefix) {
    const names = await this.page.locator('.inventory_item_name, .inventory_item .inventory_item_name').allTextContents();
    // whitelist exceptions (produk intentionally different)
    const whitelist = [
    ...
    for (const n of names) {
        ...
        if (whitelist.includes(trimmed.toLowerCase())) continue;
        assert.ok(trimmed.startsWith(prefix), `Product name "${trimmed}" does not start with "${prefix}"`);
    }
});
...
Then('products remain sorted by {string}', async function (mode) {
    const names = await this.page.locator('.inventory_item_name, .inventory_item .inventory_item_name').allTextContents();
    const pricesRaw = await this.page.locator('.inventory_item_price, .inventory_item .inventory_item_price').allTextContents();

    if (mode.toLowerCase().includes('name')) {
        const normalized = names.map(n => n.trim());
        const sorted = [...normalized].sort((a, b) => a.localeCompare(b));
        assert.deepStrictEqual(normalized, sorted, `Products are not sorted by name ascending after refresh. Actual: ${normalized.join(', ')}`);
    } else if (mode.toLowerCase().includes('price') && mode.toLowerCase().includes('low')) {
        // parse $xx.xx
        const nums = pricesRaw.map(p => parseFloat((p || '').replace(/[^0-9.]/g, '')));
        const sorted = [...nums].sort((a, b) => a - b);
        assert.deepStrictEqual(nums, sorted, `Products are not sorted by price low->high after refresh. Actual: ${nums.join(',')}`);
    } else {
        // implement other modes if necessary
        this.log && this.log(`Unhandled sorting mode: ${mode}`);
    }
});
