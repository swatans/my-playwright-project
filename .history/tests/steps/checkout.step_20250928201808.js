...
Then('I should see the checkout form', async function () {
    // Checkout form wrapper has data-test="checkout-info-container"
    const container = this.page.locator('[data-test="checkout-info-container"], #checkout_info_container');
    const visible = await container.isVisible().catch(() => false);
    // Alternatively also check title text "Checkout: Your Information"
    const title = this.page.locator('[data-test="secondary-header"] .title, .header_secondary_container .title, .title[data-test="title"]');
    const titleText = await title.first().textContent().catch(() => null);

    assert.ok(visible || (titleText && titleText.includes('Checkout')), 'Checkout form not visible');
});
