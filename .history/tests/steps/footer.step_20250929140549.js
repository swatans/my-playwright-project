...
Then('the page opens (or link exists)', async function () {
    const anchors = this.page.locator('a[href]').all();
    const terms = this.page.locator('text=Terms of Service, text=Privacy Policy, a:has-text("Terms of Service")');
...
When('I go back to inventory', async function () {
    // either click Back Home button on checkout-complete or navigate directly
    ...
    // fallback: navigate to inventory directly
