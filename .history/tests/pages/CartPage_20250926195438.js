...
class CartPage {
    ...
    async getItems() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // read name and price (await the promises)
            ...
            // read qty defensively: try to get innerText, fallback to '1' if selector not found
            const qtyText = await row.locator('[data-test="item-quantity"]').innerText().catch(() => '');
            const qty = qtyText ? qtyText.trim() : '1';
